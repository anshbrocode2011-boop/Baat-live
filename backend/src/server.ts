...
app.post("/api/chats/:id/messages", auth, async (req: AuthRequest, res) => {
  ...
  try {
    if (pool && dbAvailable) {
      ...
      // Broadcast with user-specific 'mine' calculation
...
// Dynamic static frontend serving
...
app.use((req, res, next) => {
  ...
  const fDir = findFrontendDir();
  if (fDir && fs.existsSync(fDir)) {
    express.static(fDir)(req, res, next);
  } else {
    next();
  }
});
...
app.get("*", (req, res, next) => {
  ...
  const fDir = findFrontendDir();
  const indexFile = fDir ? path.join(fDir, "index.html") : null;
...
async function start() {
  if (pool) {
    try {
      ...
      console.warn("PostgreSQL connection failed, operating with in-memory store:", err);
      dbAvailable = false;
    }
  } else {
  ...
  server.listen(port, "0.0.0.0", () => {
    console.log(`Baat backend server listening on http://0.0.0.0:${port}`);
  });
}
